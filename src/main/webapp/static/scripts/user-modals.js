...
userModals.ready = function () {

    // selectors

    var
        $groupIdInput = $('#group-id-input'),
        $preGroupNameButton = $('#pre-group-name-button'),
    ...
    handler = {
        ...
        showGroupNameModal: function () {
            $preGroupNameButton.blur();
            handler
                .doExplicitFind()
            ;
            $groupNameModal
    ...
    userModals.handler = handler;

    /* find group name */

    $preGroupNameButton
        .on('click', handler.showGroupNameModal)
        .keypress(function (e) {
            if (e.which == 13) {
                handler
                    .showGroupNameModal()
